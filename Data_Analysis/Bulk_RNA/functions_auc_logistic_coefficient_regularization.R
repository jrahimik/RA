...
#get random samples of genes and run lasso followed by logistic regression but with the regularization coefficient of max AUC
lasso_logistic_permute <- function(X, y, k, module_gene_n, reg_coef) {
  X_mt <- as.matrix(X)
  n_permute <- 1000
  require(caret)
  require(glmnet)
...
#permute for logistic regression 
logit_run_permute <- function(X, y, module_gene_n) {
  n_permute <- 1000 
  roc_data <- list()
  auc_df <- c()
  for (n in 1:n_permute) {
