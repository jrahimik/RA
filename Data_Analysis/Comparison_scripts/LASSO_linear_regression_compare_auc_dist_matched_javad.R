...
library(doParallel)
library(foreach)
source('/ix/djishnu/Priyamvada/Immport/RA_project_git/functions_auc_linear.R')
output_directory <- '/ix/djishnu/Javad/RA/Data_Analysis/results/'
...
for (m in 1:length(module_set)) {
  ...
  nonModular        <- setdiff(ldak_genes$V1,module_all)
  noModular_noldak  <- setdiff(nonModular,ldak_genes_top$V1)
  rand_mod_set[[m]] <- noModular_noldak [sample(1:nrow(ldak_genes), length(module))]
  ...
  for (m in module_types) {
    ...
    for (mod in 1:length(module_list)) {
      ...
      if (nrow(cell_metadata) <= gene_overlap) {
        ...
        lasso_linear_predict(X = expression_data_wo_phenotype, 
                             y = phenotype_labels,
                             k = k)
        ...
        phenotype_labels <- module_data$Phenotype
        k <- length(phenotype_labels)
        true_model <- linear_run_predict(X = expression_data_wo_phenotype, 
                                         y = phenotype_labels)
